...
$domain = $env['pbxtest'];//Server to reach the Client_app files
$CRMhandler = "https://".$domain."/client_app/messages/ahhsajhkcbdwolcbolwdiuhboiget_sms.php";
...
if($recipient != "" && ($message != "" || $files != "") && $sender != ""){
	...
	if($type === "MMS") { //For MMS
		...
		$out = file_get_contents($CRMhandler."?from=".base64_encode($sender)."&to=".base64_encode($recipient)."&text=".base64_encode($message) . "&messageType=SMS");
		echo $out;
